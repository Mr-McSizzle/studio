  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-headline text-foreground">
              Post-Launch Quest Log
            </h1>
            <p className="text-muted-foreground">
              Your active quarterly objectives assigned by EVE.
            </p>
          </div>
        </div>
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" className="border-accent text-accent hover:bg-accent/10 hover:text-accent">
              <Bot className="mr-2 h-5 w-5" /> Consult EVE on Objectives
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="p-0 w-full max-w-none sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl overflow-y-hidden flex flex-col">
            <SheetHeader className="p-4 border-b">
              ...
              <SheetDescription>
                Ask EVE for advice on prioritizing objectives or understanding their strategic value.
              </SheetDescription>
            </SheetHeader>
      ...
      <Card className="shadow-lg">
          <CardHeader>
            <CardTitle>Current Quarter's Objectives</CardTitle>
            <CardDescription>
              {missions.length > 0
                ? `Focus on these objectives to advance your startup. (${pendingTasks} pending)`
                : "No objectives assigned for this quarter. Advance the simulation on the Dashboard to receive new directives."}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {missions.length > 0 ? (
