...
import type { AttachmentPoint } from '@/types/guidance';
...
export const ContextualGuidanceTip: React.FC<ContextualGuidanceTipProps> = ({
  ...
  useEffect(() => {
    ...
    if (targetElement) {
      ...
      switch (attachment) {
        ...
        case 'center':
        default:
          // Default centered position already calculated
          break;
      }
    }
  ...
  }, [targetElement, attachment, isVisible, message]); // Rerun if message changes, as it affects tip size
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // For animation based on state (can't directly use data-state here from parent)
        // Parent `DynamicGuidanceSystem` will control overall visibility including initial animation
        // This component's opacity is for fine-tuning after initial mount/measurement
    ...
    >
      <Button
        ...
        onClick={onClose}
        aria-label="Close guidance tip"
      >
        <X className="h-4 w-4" />
      </Button>
      <div className="flex items-start gap-3">
        <Lightbulb className="h-6 w-6 text-accent mt-1 shrink-0" />
        <div className="prose prose-sm dark:prose-invert max-w-none text-popover-foreground">
